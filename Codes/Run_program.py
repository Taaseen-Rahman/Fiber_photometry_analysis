...
if inputs["Settings"] == True:
    ...
    # Import the options for analysis from a settings excel file.
    ...
    if inputs['Analysis'] == 'FED3':
        inputs = find_options_FED3(inputs)
        inputs = choose_type_FED3_task(inputs)
        
        if inputs['Task type'] == 'Normal task':
            inputs = find_event_names(inputs)
            inputs = choose_FED3_options(inputs)
        if inputs['Task type'] == 'Pavlovian task':
            inputs = find_event_names_Pavlovian(inputs)
            inputs = choose_FED3_Pavlovian_options(inputs)
